...
2. **Candidate 2 — day totals.** Deepen the tracking module with a day-total/elapsed interface consumed by both the sessions toolbar and the reports module; restore `seconds_between` to pure time math. Kills the duplicated aggregation in `work_items_tab._refresh_sessions` vs `generate_report`.
